...
Одинадцятий крок. У які канали лідгену йти — **під стадію + SMF-діру (зазвичай L) + де реально буває ICP +
buyer journey**. Recommendations only (не запускаємо канали тут). Без гейту.

Вхід — 02 (стадія + SMF), 03 (персона: preferred channels / де тусуються), 09 (шлях клієнта: який канал на
яку стадію), 10 (чи готові матеріали під канал). Прочитай `${CLAUDE_PLUGIN_ROOT}/reference/agency-stages.md`
...
- **Адаптуй канали під КОНКРЕТНИЙ ICP — називай реальні майданчики, не абстрактні категорії.** Замість
  ...
  саме ця персона (з 03 persona «де тусуються» + 09 journey + `WebSearch`). **Якщо ICP реально там —
  додай цю спільноту окремим каналом у список.** Без конкретики канал неактивовний.
- Завжди спирайся на попередні кроки: персона (03), шлях клієнта (09), готовність матеріалів (10).
- Канал годує **awareness/business-trigger** стадії шлях клієнта (09); пізні стадії — sales/контент.
...
## Вихід (Notion)
Під-сторінка **«11 · Channels Plan»**: scored shortlist · per-channel картки (з data sources) ·
