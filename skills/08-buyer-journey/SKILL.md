---
name: 08-buyer-journey
description: Step 8 of the GTM flow. Map the buyer's journey for the chosen persona — stages from unaware to advocacy (unaware -> problem-aware -> solution-aware -> vendor consideration -> decision/pilot -> onboarding -> expansion) with, per stage, the buyer mindset/questions, the trigger that advances them, the main objection, the touchpoint/content that fits, and the channel. Feeds materials (09) and channels (10). Stage-adaptive. Use when the user asks buyer journey, customer journey, шлях покупця, awareness consideration decision, funnel stages, touchpoints. Writes to Notion in Ukrainian.
argument-hint: "(reads persona + VP + competitor + offers from steps 3-7 in Notion)"
---

# 08 · Buyer's Journey

Восьмий крок. Карта шляху покупця для обраної персони — від «не усвідомлює» до «адвокат». На кожній
стадії: що думає/питає, що тригерить перехід далі, головне заперечення, доречний тачпоінт/контент, канал.
Це **міст до 09 (матеріали) і 10 (канали)** — їх будуємо проти цієї карти. Без гейту.

Вхід — 03 (персона/JTBD), 06 (VP, dream outcome, objections), 05 (alternatives), 07 (offers/ladder).
Прочитай `${CLAUDE_PLUGIN_ROOT}/reference/agency-stages.md`.

## Реюз
`deep-company-analyser` (customer language per стадія) · buying-process поля з VP-канвасу (06).
...
- **Тачпоінт / контент** (що тут доречно — годує крок 09).
- **Канал** (де відбувається — годує крок 10).
...
- Заперечення беруться з 06 (VP) і реальних альтернатив (05).
...
## Вихід (Notion)
Під-сторінка **«08 · Buyer's Journey»**: таблиця стадій × 6 полів + примітка, що годує 09/10 +
...
- Mindset/заперечення — у customer language (з 06/05), не вигадані.
...
- Тачпоінти/канали позначені як вхід для 09/10. Notion-сторінка створена.
