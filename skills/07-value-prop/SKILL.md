...
# 07 · Value Proposition (VP Canvas)

Сьомий крок. Будуємо VP як **канвас, що ПОЧИНАЄТЬСЯ з проблеми ринку** (не з нашого рішення). Логіка
...
Вхід — **06 (positioning: анкер/ворог/категорія/statement — VP будуй ВІД нього)**, 01 (verbatim/болі),
03 (ринок/персона/JTBD), 05 (competitor/whitespace), за наявності 08 (offers). Формат — VP-канвас Viktor'а
(canvas-style). Прочитай `${CLAUDE_PLUGIN_ROOT}/reference/agency-stages.md`.
**GTM enrichment** (`${CLAUDE_PLUGIN_ROOT}/reference/gtm-frameworks.md` §4): Value Prop Canvas
(JTBD/Pains/Gains → mission-critical relievers, ≥10 інтерв'ю) · UVP = **diff value** · USP-тест
(valued AND unique; **повна позиція — крок 06**) · A/B-тест меседжу (≥50 конв., тиждень) · Cialdini.
...
**F · Синтез.**
- *Positioning category* (з кроку 06 — не вигадуй заново; named) + anchors.
...
## Вихід (Notion)
Під-сторінка **«07 · Value Proposition»**, порядок: **0·Опис → VP-statements (Big Promise) →
