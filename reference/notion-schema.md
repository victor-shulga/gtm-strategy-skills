...
## ⚠️ Технічна примітка (форматування)
- **Не використовуй `notion-update-page insert_content` для багаторядкового markdown** — він робить `\n`
  літеральним «n» (таблиці/булети злипаються). Для нового контенту бери **`notion-create-pages`** (нова
  сторінка) або **`replace_content`** (повна заміна — обидва коректно обробляють переноси).
- `replace_content` **не можна** на сторінці з під-сторінками (видалить їх) — там лише `update_content`
  (точкові заміни) або **однорядкові** вставки.
- Перед масовим записом — переконайся, що markdown-таблиці на окремих рядках з реальними переносами.
